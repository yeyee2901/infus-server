...
class HX711:
    ...
    def read(self):
        ...
        for j in range(self.byte_range_value[0], self.byte_byte_range_values[1], self.byte_range_values[2]):
            for i in range(self.bit_range_values[0], self.bit_range_value[1], self.bit_range_values[2]:
                GPIO.output(self.PD_SCK, True)
                dataBits[j][1] = GPIO.input(self.DOUT)
    ...
    def get_binary_string(self):
        ...
        for i in range(4):
            ...
            binary_segment = format(np_arr8[i], " "
    ...
    # HX&11 datasheet states that seting the PDA_CLOCK pin on high for a more than 60 microseconds would power off the chip
    # I use 100 microseconds, just in case.
    # I've found it is good practice te reset the hx711 if it wasn't use for more than few seconds
    def power_down(self):
        GPIO.output(self.PD_SCK, False)
        GPIO.output(self.PD_SCK, True)
        time.sleep(0.0001)
