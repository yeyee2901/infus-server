...
class app(Frame):
    def __init__(self):
        Frame.__init__(self)
        self.option_add('*Font','arial 20 bold') #besar ukuran font display
        self.pack(expand=YES, fill=BOTH)
        self.master.title('Calculator')#title pada display

        display = StringVar()
        Entry(self, relief=RIDGE
              ,textvariable=display,justify='right', bd=30, bg="dark blue").pack(side=TOP, expand=YES, fill=BOTH)
